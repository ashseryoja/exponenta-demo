...
function ProductScene({onReady, intro, spin, reset, interactive}) {
 ...
 useEffect(()=>{
  ...
  new GLTFLoader().load(A+'zuno-cup.glb',gltf=>{
 ...
 return <div className="product-canvas" ref={mount} aria-label="ZUNO-ի եռաչափ բաժակ։ Պտտելու համար քաշեք կամ օգտագործեք սլաքները։ Հպվեք՝ պտտելու համար։" role="img" tabIndex={0} />;
}
...
export function App(){
 ...
 return <>
  ...
  <header className="header"><a className="brand" href="#home" aria-label="ZUNO — գլխավոր էջ"><img src={A+'zuno-mark.svg'} alt=""/><span>ZUNO</span></a><nav aria-label="Նավարկում"><a href="#taste">Բացահայտիր համը</a><a href="#rhythm">Քո ռիթմը</a></nav><a className="social-link" href="#rhythm">Տես կոնցեպտը <ArrowUpRight size={19}/></a></header>
  <main>
  <div className="product-stage">
   {loaded!=='error'&&<ProductScene onReady={setLoaded} intro={intro} spin={spin} reset={reset} interactive={!intro}/>}
   {loaded==='error'&&<div className="model-fallback"><img src={A+'zuno-mark.svg'} alt="ZUNO-ի նշանը"/><span>Այս դիտարկիչում 3D-ն հասանելի չէ</span></div>}
   <section id="home" className={`hero ${intro?'':'hero-entered'}`}>
    ...
    <div className="hero-product-backdrop" aria-hidden="true"><div className="hero-disc"/><Sparkle className="hero-spark hero-spark-one" weight="fill"/><Sparkle className="hero-spark hero-spark-two" weight="fill"/><span className="hero-product-tag">ZUNO / SWIRL</span></div>
